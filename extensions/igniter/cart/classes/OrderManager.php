...
class OrderManager
{
    ...
    public function saveOrder($order, array $data)
    {
     //dd($data);
        ...
        $order->fill($data);
        $order->address_id = $addressId; //Add table ID
        ...
        if (!empty($order->table_id)) {
            $table = Tables_model::find($order->table_id);
            if ($table) {
                $table->table_status = false; //false is "unavailable" or 0
                $table->save();
            }
        }
    ...
    public function updateOrderDetails($orderId, array $data)
    {
        ...
        // Only allow updating certain fields to prevent accidental modifications
        $order->fill($data);
        $order->status_id = 5;
        $order->processed = 1;
        $order->save();

        // Optional: update related fields, e.g., table availability
        if (!empty($order->table_id)) {
            $table = Tables_model::find($order->table_id);
            //dd($table);
            if ($table) {
    ...
    public function editOrderWithoutCheckout($orderId)
    {
        $order = Orders_model::find($orderId);
        if (!$order) {
            throw new ApplicationException('Order not found.');
